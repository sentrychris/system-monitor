...
<template>
  ...
  <table v-else class="table table-sm table-striped table-responsive-sm">
    <thead>
      <tr v-if="nested">
        <th class="cursor-pointer" v-for="(header, key) in Object.keys(tableData)" :key="key" @click="changeOrder(header)">
          <span class="me-1">
            <font-awesome-icon v-if="tableOrder === 'asc' && tableSortKey === header"
      ...
      <tr v-else>
        <th class="cursor-pointer" v-for="(header, key) in Object.keys(tableData[0])" :key="key" @click="changeOrder(header)">
